...
class SuspendedError extends Error {
    constructor() { super('Your account has been suspended') }
}
...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
            if (skipSync.current) { setLoading(false); return }
    ...
    const register = async (email: string, password: string, name: string) => {
        ...
        try {
            ...
            const message = error?.response?.data?.message ?? (err instanceof Error ? err.message : undefined)
    ...
    return (
        <AuthContext.Provider value={{
            ...
            logout, forgotPassword, refreshDbUser,
        }}>
            {!loading && children}
